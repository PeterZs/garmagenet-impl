...
from  ..models.download import load_checkpoint
...
class PointNetClassifier(FeatureExtractor):
    ...
    def get_features(self, point_clouds):
        """
        Args:
            point_clouds: BatchSize X 采样点数量 X 3
        Returns:
        ...
        with torch.no_grad():
            _, _, features = self.models[0](torch.tensor(point_clouds, dtype=torch.float32, device="cuda").permute(0, 2, 1), features=True)
