...
class SurfZData(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.cache_fp = os.path.join(
            ...
            'surfz_%s.pkl'%('validate' if validate else 'train')
        )
        # [test]
        print("dataset init ")
        ...
        # Config data chunks
        if args.chunksize > 0:
            self.chunksize = min(args.chunksize, len(self.data_list))
            if self.validate: self.chunksize = self.chunksize // 8
            self.data_chunks = [self.data_list[i:i+self.chunksize] for i in range(0, len(self.data_list), self.chunksize)]
            self.chunk_idx = -1
            print('Data chunks: num_chunks=%d, chunksize=%d.'%(len(self.data_chunks), self.chunksize))
    ...
    def __encode_chunk__(self):

        print('Encode chunk [%03d/%03d]: '%(self.chunk_idx, len(self.data_chunks)), self.cache['surf_pos'].shape)
    ...
    def __next_chunk__(self, lazy=False):
        ...
        if not hasattr(self, 'data_chunks'): return
        if lazy and np.random.rand() < 0.5: return
...
class SurfInpaintingData(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        print('Loading %s data...'%('validation' if validate else 'training'))
        with open(input_list, 'rb') as f: self.data_list = pickle.load(f)['val' if self.validate else 'train'][:100] #todo del [:XXX]
        if use_data_root:
            self.data_list = [os.path.basename(x) for x in self.data_list
                              if os.path.exists(os.path.join(self.data_root, os.path.basename(x)))]
        random.shuffle(self.data_list)
    ...
    def init_encoder(self, geo_mask_encoder, mask_encoder, z_scaled=None):
        print("Init mask_geo_encoder and mask_encoder...")
        self.geo_mask_encoder = geo_mask_encoder
    ...
    def __init_zero_latent__(self):
        raise NotImplementedError("这一部分还是旧的，没有更新")

        if hasattr(self, 'zero_latent'): return

        try:
            z_device = self.geo_mask_encoder.module.parameters().__next__().device
        except:
            z_device = self.geo_mask_encoder.parameters().__next__().device
    ...
    def __pad_latents__(self, surf_pos, latent_geo_mask, latent_mask, surf_cls):
        ...
        if self.padding == 'repeat':
            ...
            pad_idx = torch.randint(0, n_surfs, (n_pads,))
            surf_pos = torch.cat([surf_pos, surf_pos[pad_idx, ...]], dim=0)[rand_idx, ...]
            latent_geo_mask = torch.cat([latent_geo_mask, latent_geo_mask[pad_idx, ...]], dim=0)[rand_idx, ...]
            latent_mask = torch.cat([latent_mask, latent_mask[pad_idx, ...]], dim=0)[rand_idx, ...]
            surf_cls = torch.cat([surf_cls, surf_cls[pad_idx, ...]], dim=0)[rand_idx, ...]
            pad_mask = torch.cat([
                torch.zeros(n_surfs, dtype=bool), torch.ones(n_pads, dtype=bool)
                ], dim=0)[rand_idx, ...]
            ...
            latent_geo_mask = torch.cat([
                latent_geo_mask[pad_idx, ...], torch.zeros((n_pads, *latent_geo_mask.shape[1:]), dtype=latent_geo_mask.dtype, device=latent_geo_mask.device)
            ], dim=0)
            latent_mask = torch.cat([
                latent_mask[pad_idx, ...], torch.zeros((n_pads, *latent_mask.shape[1:]), dtype=latent_mask.dtype, device=latent_mask.device)
            ], dim=0)
            surf_cls = torch.cat([
                surf_cls[pad_idx, ...], torch.zeros((n_pads, *surf_cls.shape[1:]), dtype=surf_cls.dtype, device=surf_cls.device)-1
            ], dim=0)
            ...
            latent_geo_mask = torch.cat([
                latent_geo_mask[pad_idx, ...], self.zero_latent.repeat(n_pads, 1)
            ], dim=0)
            latent_mask = torch.cat([
                latent_mask[pad_idx, ...], self.zero_latent.repeat(n_pads, 1)
            ], dim=0)
            surf_cls = torch.cat([
                surf_cls[pad_idx, ...], torch.zeros((n_pads, *surf_cls.shape[1:]), dtype=surf_cls.dtype, device=surf_cls.device)-1
            ], dim=0)
        ...
        return surf_pos, latent_geo_mask, latent_mask, surf_cls, pad_mask
    ...
    def __load_and_encode_all__(self):
        ...
        if os.path.exists(self.cache_fp):
            ...
            if not hasattr(self, 'z_scaled'): self.z_scaled = 1.0 / (self.cache['latent'].std() + 1e-6)
            if self.padding == 'zerolatent' and not hasattr(self, 'zero_latent'): self.__init_zero_latent__()
            print('Load cache from %s: '%(self.cache_fp), "surf_pos: ", self.cache['surf_pos'].shape,
                  "latent_geo_mask",  self.cache['latent_geo_mask'].shape,
                  "latent_mask", self.cache['latent_mask'].shape,
        ...
        for data_id in tqdm(self.data_list):
            ...
            data_fp = os.path.join(self.data_root, data_id)
            try:
                surf_pos, surf_ncs, surf_cls, caption, data_fp_orig = self.__load_one__(data_fp)
                assert surf_pos.shape[0] <= self.max_face
                with torch.no_grad():
                    latent_geo_mask = self.geo_mask_encoder(surf_ncs.permute(0, 3, 1, 2).to(z_device)).flatten(start_dim=1)
                    latent_mask = self.mask_encoder(surf_ncs[...,-1,None].permute(0, 3, 1, 2).to(z_device)).flatten(start_dim=1)
                start_idx = end_idx
                end_idx = start_idx + surf_pos.shape[0]
                ...
                cache['item_idx'].append((start_idx, end_idx))
                cache['data_id'].append(data_id)
                cache['latent_geo_mask'].append(latent_geo_mask.to(surf_pos.device).to(surf_pos.dtype))
                cache['latent_mask'].append(latent_mask.to(surf_pos.device).to(surf_pos.dtype))
                cache['data_fp'].append(data_fp_orig)
        ...
        cache['surf_pos'] = torch.cat(cache['surf_pos'], dim=0)
        cache['latent_geo_mask'] = torch.cat(cache['latent_geo_mask'], dim=0)
        cache['latent_mask'] = torch.cat(cache['latent_mask'], dim=0)
        cache['surf_cls'] = torch.cat(cache['surf_cls'], dim=0)

        self.cache = cache
        print('Saving cache from %s: ' % (self.cache_fp), "surf_pos: ", self.cache['surf_pos'].shape,
              "latent_geo_mask", self.cache['latent_geo_mask'].shape, self.cache['latent_geo_mask'].std(),
              "latent_mask", self.cache['latent_mask'].shape, self.cache['latent_mask'].std(),
              'z_scaled: ', self.z_scaled)
        os.makedirs(os.path.dirname(self.cache_fp), exist_ok=True)
        with open(self.cache_fp, 'wb') as f: pickle.dump(cache, f)
        ...
        del self.geo_mask_encoder
        del self.mask_encoder

        torch.cuda.empty_cache()

        print('Encode all data: ', self.cache['latent_geo_mask'].shape, self.cache['surf_pos'].shape)
    ...
    def __getitem__(self, index):
        ...
        surf_pos = self.cache['surf_pos'][start_idx:end_idx, ...] * self.bbox_scaled
        latent_geo_mask = self.cache['latent_geo_mask'][start_idx:end_idx, ...] * self.z_scaled
        latent_mask = self.cache['latent_mask'][start_idx:end_idx, ...] * self.z_scaled

        surf_cls = self.cache['surf_cls'][start_idx:end_idx, ...]

        surf_pos, latent_geo_mask, latent_mask, surf_cls, pad_mask = (
            self.__pad_latents__(surf_pos, latent_geo_mask, latent_mask, surf_cls))
        ...
        # [todo] 返回 2dBBox、MaskLatent、3dBBox、GeoMaksLatent

        return (
            surf_pos, latent_geo_mask, latent_mask, pad_mask, surf_cls, caption
        )