...
def process_data(
    ...
    if uuid in uuid2caption:
        caption = uuid2caption[uuid]
...
def process_item(data_idx, data_item, args, glctx, uuid2caption):
    # try:
...
def main(args):
    ...
    uuid2caption = {}
    with open(os.path.join(os.path.dirname(args.input), "captions.csv"), encoding="utf-8") as f:
        reader = csv.DictReader(f, fieldnames=["uuid", "text"])
        for row in reader:
            uuid2caption[row["uuid"]] = row["text"]
    ...
    with open(log_file, 'a+') as f:
        with ThreadPoolExecutor(max_workers=1) as executor:  # 可以调整max_workers以改变并行度
            futures = {executor.submit(
                process_item, data_idx, data_item, args, glctx, uuid2caption): data_item for data_idx, data_item in enumerate(data_items)}
